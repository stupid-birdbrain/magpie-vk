...
public unsafe struct VulkanInstance : IDisposable {
    ...
    public VulkanInstance(VkCtx libraryContext, string appName, string engineName) {
        ...
        if (inputLayers.Any()) {
            if (!requiredExtensions.Contains(new VkUtf8String(VK_EXT_DEBUG_UTILS_EXTENSION_NAME.GetPointer()).ToString())) {
                requiredExtensions.Add(new VkUtf8String(VK_EXT_DEBUG_UTILS_EXTENSION_NAME.GetPointer()).ToString());
            }
        }
        ...
        var result = vkCreateInstance(&instanceCreateInfo, null, out Value);
        ThrowIfInstanceCreationFailed(result);
                
        vkLoadInstanceOnly(Value);
                
        if (inputLayers.Any()) {
            VkResult createMessengerResult = vkCreateDebugUtilsMessengerEXT(Value, debugUtilsCreateInfo, null, out VkDebugUtilsMessengerEXT createdMessenger);
